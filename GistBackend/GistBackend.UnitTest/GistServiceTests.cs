...
public class GistServiceTests
{
    ...
    private static readonly List<RssEntry> TestRssEntries = [
        new (
            "test reference",
            1,
            "test author",
    ...
    private static readonly List<AIResponse> TestAIResponses = [
    ...
    private static readonly List<Gist> TestGists =
        TestRssEntries.Zip(TestAIResponses).Select((tuple, i) => new Gist(
            entry: tuple.First,
            aiResponse: tuple.Second
        ) { Id = i }).ToList();
